...
public:
	...
	bool addGravityInZ(double dt);
	bool divFree(double error);
...
bool StaggeredGrid::addGravityInZ(double dt) {
	unsigned int i,j,k;
...
bool StaggeredGrid::divFree(double error) {
	unsigned int i,j,k;
	double div = 1;
	double beta = 1.5;
	while (div > error) {
		for (i=1; i<nx-2; i++) {
			for (j=1; j<ny-2; j++) {
				for (k=1; k<nz-2; k++) {
					...
					u(i+1,j,k) -= beta * div / 6;
					u(i,j,k) += beta * div / 6;
					v(i,j+1,k) -= beta * div /6;
					v(i,j,k) += beta * div /6;
					w(i,j,k+1) -= beta * div/6;
					w(i,j,k) += beta * div / 6;
				}
			}
		}
